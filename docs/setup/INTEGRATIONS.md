...
- **Stripe Connect**: Growth owners connect their own Standard Stripe account (OAuth or hosted onboarding). Stripe handles KYC.
...
Owners connect **their own Stripe Standard account**. Stripe hosts identity verification and payouts. SiteSprintz never collects KYC documents and never charges the platform Stripe account.

If `STRIPE_CLIENT_ID` is set, Connect uses Stripe OAuth so owners can reuse an existing Stripe account. Otherwise SiteSprintz opens Stripe-hosted Standard onboarding.

**Endpoint:** `POST /api/connect/onboard`

**Purpose:** Start Standard Connect (OAuth or hosted onboarding)
...
Square and PayPal use the same pattern: `GET /api/connect/square` and `GET /api/connect/paypal` return a provider authorize URL. Do not paste API secrets.

**Endpoint:** `GET /api/connect/status`

**Purpose:** Check Connect account status plus Square/PayPal
