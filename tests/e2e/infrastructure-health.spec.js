...
test.describe('Database Schema Health Checks', () => {
  ...
  test('should verify database is connected', async ({ request }) => {
    ...
    // server.js doesn't currently return database status in /health response
  ...
  test('should verify users table has required columns', async ({ request }) => {
    ...
    if (!response.ok()) {
      console.log('User Health 400 Error Body:', await response.json().catch(() => 'No body'));
    }
  ...
  test('should verify sites table structure', async ({ request }) => {
    ...
    // Note: Guest publish might be rate limited or require CSRF depending on middleware.
    // Assuming it needs CSRF now if it's a POST.
    const csrfToken = await getCsrfToken(request);
...
test.describe('Authentication Infrastructure', () => {
  ...
  test('should verify login endpoint works', async ({ request }) => {
    ...
    const loginResponse = await request.post(`${API_URL}/api/auth/login`, {
      headers: { 'X-CSRF-Token': csrfTokenLogin },
      data: { email, password }
    });
